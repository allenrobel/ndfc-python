...
```bash
./policy_info_switch.py --config config/policy_info_switch.yaml --nd-username admin --nd-password MyPassword --nd-domain local --nd-ip4 10.1.1.2
```
"""
...
def action(cfg: PolicyInfoSwitch) -> None:
    ...
    for policy in instance.policies:
        result_msg += f"{json.dumps(policy, indent=4, sort_keys=True)}"
    log.info(result_msg)
    print(result_msg)
...
def setup_parser() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Retrieve policies for a switch.",
    )
