...
3. Edit ./examples/config/maintenance_mode_info.yaml with desired values
...
./examples/maintenance_mode_info.py \
    --config ./examples/config/maintenance_mode_info.yaml \
    --nd-domain local \
    --nd-ip4 10.1.1.1 \
...
from ndfc_python.validators.maintenance_mode import MaintenanceModeInfoConfigValidator
...
if True in task.rest_send.results.failed:  # type: ignore[attr-defined]
    err_msg = "unable to query maintenance mode"
    log.error(err_msg)
    print(err_msg)
